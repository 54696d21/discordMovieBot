...
def evalMovie(requestMovieName):
    try:
        ...
        movieRatingClass = result.get("movies")[0].get("meterClass")  # certified_fresh
        movieRatingScore = result.get("movies")[0].get("meterScore")  # certified_fresh
        ...
        retStr = (
            ...
            + "' from the year "
            + str(movieYear)
            + " . Its rating on Rotten Tomatoes is "
            + str(movieRatingScore)
        )  # + " | " + str(movieRatingClass)
